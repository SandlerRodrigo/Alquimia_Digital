...
float distanciaEuclidianaQuadrado(RGBpixel, RGBpixel);
...
int main(int argc, char *argv[])
{
    ...
    // Cria um array de pixelInfo para cada imagem
    pixelInfo *origemInfo = malloc(tam * sizeof *origemInfo);
    pixelInfo *desejInfo = malloc(tam * sizeof(pixelInfo));

    // Copia os pixels das imagens originais para os arrays de PixelInfo
    for (int i = 0; i < tam; i++)
    {
        origemInfo[i].pixel = pic[ORIGEM].pixels[i];
        origemInfo[i].index = i;
        // origemInfo[i].used = 0; // descomentar para a solução 1

        desejInfo[i].pixel = pic[DESEJ].pixels[i];
        desejInfo[i].index = i;
    }
    ...
    // // Para cada pixel na imagem desejada
    // for (int i = 0; i < tam; i++)
    // {
    //     int closestIndex = -1;
    ...
    //     for (int j = 0; j < tam; j++)
    //     {
    //         if (!origemInfo[j].used)
    //         {
    //             float distance = distanciaEuclidianaQuadrado(desejInfo[i].pixel, origemInfo[j].pixel);

    //             // Se a distância entre os pixels for menor que a menor distância encontrada até agora
    ...
    //     // Marca o pixel da imagem original como usado
    //     origemInfo[closestIndex].used = 1;

    //     // Atribui o pixel da imagem original à posição correspondente na imagem de saída
    //     pic[SAIDA].pixels[desejInfo[i].index] = origemInfo[closestIndex].pixel;
    // }
    ...
    // Ordena os pixels na imagem desejada por intensidade
    qsort(desejInfo, tam, sizeof(pixelInfo), compareIntensity);

    // Ordena os pixels na imagem original por intensidade
    qsort(origemInfo, tam, sizeof(pixelInfo), compareIntensity);

    // Para cada pixel na imagem desejada
    for (int i = 0; i < tam; i++)
    {
        // Atribui o pixel da imagem original à posição correspondente na imagem de saída
        pic[SAIDA].pixels[desejInfo[i].index] = origemInfo[i].pixel;
    }
...
// Optamos por não usar a função abaixo, pois ela não é necessária para o algoritmo final
float distanciaEuclidianaQuadrado(RGBpixel a, RGBpixel b)
{
    // Calculate the difference between the RGB values of the pixels
    float rDiff = a.r - b.r;
    float gDiff = a.g - b.g;
    float bDiff = a.b - b.b;
...
// Função de comparação para qsort: compara a intensidade dos pixels
int compareIntensity(const void *a, const void *b)
{
    // Casta os ponteiros de void para ponteiros de pixelInfo
    pixelInfo *pixelA = (pixelInfo *)a;
    pixelInfo *pixelB = (pixelInfo *)b;

    // Calcula a intensidade dos pixels
    float intensityA = pixelA->pixel.r + pixelA->pixel.g + pixelA->pixel.b;
    float intensityB = pixelB->pixel.r + pixelB->pixel.g + pixelB->pixel.b;

    // Retorna a diferença entre as intensidades
